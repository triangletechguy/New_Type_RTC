...
function waitForSocketConnection(socket, timeoutMs = 8000) {
...
function joinSignalingRoom(socket, payload, timeoutMs = 8000) {
  return new Promise((resolve, reject) => {
    socket.timeout(timeoutMs).emit('join-room', payload, (error, response) => {
      if (error) {
        reject(new Error('Signaling room join timed out.'))
        return
      }

      if (!response?.ok) {
        reject(new Error(response?.message || 'Signaling room join failed.'))
        return
      }
...
function emitMediaState(socket, payload, timeoutMs = 3000) {
  return new Promise((resolve, reject) => {
    if (!socket?.connected) {
    ...
    socket.timeout(timeoutMs).emit('media-state-change', payload, (error, response) => {
      if (error) {
        reject(new Error('Media state signaling timed out.'))
        return
      }

      if (!response?.ok) {
        reject(new Error(response?.message || 'Media state signaling failed.'))
        return
      }
...
function VideoTile({ stream, label, muted = false, badge, micOn = true, cameraOn = true, rtcMode = 'video', showMediaState = false }) {
...
function ChatPanel({ roomId, signalingRoom, socket, user, room }) {
