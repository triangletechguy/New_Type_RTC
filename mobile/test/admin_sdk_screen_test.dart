...
import 'package:rtc_enterprise_mobile/screens/sdk_docs_screen.dart';
import 'package:rtc_enterprise_mobile/services/api_client.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  testWidgets('native admin creates SDK access and reviews package requests', (
    tester,
  ) async {
    ...
    await tester.tap(find.text('SDK Access'));
    await tester.pumpAndSettle();
    await tester.ensureVisible(find.text('Generate SDK access'));
    await tester.tap(find.text('Generate SDK access'));
    await tester.pumpAndSettle();

    expect(api.createdClientApps, hasLength(1));
    expect(api.createdClientApps.single['tenantId'], 3);
  ...
  testWidgets('native SDK docs expose tabbed samples and reference sections', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: ThemeData.dark(useMaterial3: true),
        home: SdkDocsScreen(api: _FakeAdminApi(), onBack: () {}),
      ),
    );
    await tester.pumpAndSettle();

    expect(find.text('Developer Docs'), findsOneWidget);
    expect(find.text('Six-step integration path'), findsOneWidget);
    expect(find.text('Tabbed SDK code samples'), findsOneWidget);

    await tester.tap(find.text('Server'));
    await tester.pumpAndSettle();

    expect(find.textContaining("app.post('/rtc/token'"), findsOneWidget);

    expect(find.text('RTC token claims'), findsWidgets);
    expect(find.text('Platform and client APIs'), findsWidgets);
    expect(find.text('Errors, webhooks, media roadmap'), findsWidgets);
  });

  testWidgets('native admin and SDK show backend error states', (tester) async {
    ...
    expect(find.text('Could not load admin'), findsOneWidget);
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: ThemeData.dark(useMaterial3: true),
        home: SdkDocsScreen(api: _FailingAdminApi(), onBack: () {}),
      ),
    );
    await tester.pumpAndSettle();

    expect(find.text('Attention'), findsOneWidget);
    expect(find.text('SDK config offline'), findsOneWidget);
  });
}
...
class _FakeAdminApi extends ApiClient {
  ...
  Future<Map<String, dynamic>> adminCreateClientApp({
    ...
    return const {
      'message': 'Flutter SDK App SDK access generated.',
      'credentials': {
        'app_key': 'app_native',
        'api_key': 'rtc_api_native',
        'sdk_token': 'rtc_sdk_native',
      },
    };
  }
...
class _FailingAdminApi extends _FakeAdminApi {
  ...
  @override
  Future<Map<String, dynamic>> rtcConfig() async {
    throw _error('/rtc/config', 'SDK config offline');
  }
}
