...
async function getUserPrivacySettings(userId, tenantId = null) {
  await ensureUserPrivacySettingsSchema()

  const tenantClause = tenantId === null || tenantId === undefined ? '' : 'AND tenant_id = :tenantId'
  ...
  return rows.length ? formatUserPrivacySettings(rows[0]) : null
}
